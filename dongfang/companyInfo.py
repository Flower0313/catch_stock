...
def getStocks():
    stocksUrl = {}
    cursor = conn.cursor()
    cursor.execute("select `code`,market from spider_base.df_a_stock_detail where ds='2022-11-22' and code='300454'")
...
base_sql = '''
           insert into spider_base.df_a_stock_company_info(`code`,event_type,specific_eventtype,l1_comment,l2_comment,notice_date)
 VALUES("{}","{}","{}","{}","{}","{}")
'''

base_sql2 = '''
    insert into spider_base.df_a_stock_shareholder(`code`,holder_rank,end_date,holder_name,shares_type,hold_num,hold_num_ratio,hold_change,change_ratio,change_reason) 
    values("{}","{}","{}","{}","{}","{}","{}","{}","{}","{}")
'''
...
def insertSQL(stock, url):
    ...
    content = real_json['dstx']['data']
    content = None
    holder = real_json['sdgdcgbd']

    cursor = conn_result.cursor()
    if content is not None:
        for i in content:
            for j in i:
                if str(j.get('EVENT_TYPE')) == '融资融券':
                    e_sql = base_sql.format(stock, j.get('EVENT_TYPE'), j.get('SPECIFIC_EVENTTYPE'),
                                            re.sub(r"\"", '\'', str(j.get('LEVEL1_CONTENT'))),
                                            re.sub(r"\"", '\'', str("|".join(j.get('LEVEL2_CONTENT')) if j.get(
                                                'LEVEL2_CONTENT') is not None else '')),
                                            j.get('NOTICE_DATE'))
    ...
    if holder is not None:
        for i in holder:
            s_sql = base_sql2.format(stock, i.get('HOLDER_RANK'), i.get('END_DATE'), i.get('HOLDER_NAME'),
                                     i.get('SHARES_TYPE')
                                     , i.get('HOLD_NUM'), i.get('HOLD_NUM_RATIO'), i.get('HOLD_CHANGE')
                                     , i.get('CHANGE_RATIO'), i.get('CHANGE_REASON'))
            print(s_sql)
