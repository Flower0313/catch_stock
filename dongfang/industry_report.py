...
def insertSQL(date):
    ...
    base_sql = '''
    insert into spider_base.df_a_stock_industry_report(`code`,`name`,report_content,east_rank,rating_change,report_count,ds) values("{}","{}","{}","{}","{}","{}","{}")
    '''
    ...
    for i in content:
        e_sql = base_sql.format('BK' + str('0' if len(i['industryCode']) == 3 else '') + i['industryCode'],
                                i['industryName'], i['title'], i['emRatingValue'], i['ratingChange'], i['count'],
                                str(i['publishDate']).split(' ')[0])
